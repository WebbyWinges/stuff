...
import {BASE_URL} from "../../utils/constants"
    ...
    export const loginUser = createAsyncThunk('users/loginUser', async (payload, thunkAPI) => {
        try {
            ...
            const login = await await axios.get(`${BASE_URL}/auth/profile`, {
                headers: {
...
const userSlice = createSlice({
    ...
    reducers:{
        ...
        addItemToFav: (state, { payload }) => {
            let newFaw = [...state.favourite];

            const found = state.favourite.find(({id}) => id === payload)

             if(found) {
                newFaw = newFaw.map((item) => {
                    return item.id === payload.id ? {...item} : item;
                })
             }
             else newFaw.push({...payload})

             state.favourite = newFaw;
        },
    },
